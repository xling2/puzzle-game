...
export default class SixteenPics extends Component<{}> {
    ...
    _start() {
        ...
        this.croper = new Croper(_IMG);
        this.croper.crop(4, 4).then((json) => {
    ...
    _upset(arr) {
        ...
        let shuffle = () => {
            //switch
            let count = parseInt(10 + Math.random() * 10) * 4;//switch 40+80 times
            for (let i = 0; i < 3; i++) {
                let hold = null;
    ...
    _getActBox() {
        ...
        let findElement = (r, c) => {
            return this.state.imgList.find((element) => {
                ...
                return element.currentIndex == (r * 4 + c);
            })
        };

        //position
        this.actBox.up = c_r > 0 ? findElement(c_r - 1, c_c) : {};
        this.actBox.down = c_r < 3 ? findElement(c_r + 1, c_c) : {};
        this.actBox.left = c_c > 0 ? findElement(c_r, c_c - 1) : {};
        this.actBox.right = c_c < 3 ? findElement(c_r, c_c + 1) : {};
                    ...
                    {this.state.imgList.map((img, index) => {
                        ...
                        return (
                            <Image style={{
                                ...
                                top: parseInt(img.currentIndex / 4) * img.fullWidth * 0.93,
                                left: img.currentIndex % 4 * img.fullWidth * 0.93,
                                borderWidth: 1,
                                borderColor: "#F9690E"
                            }} key={index}
                                   source={{uri: img.uri}}/>
